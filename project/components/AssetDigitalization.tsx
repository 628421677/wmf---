...
import { BuildingAsset, RoomAsset, UserRole } from '../types';
import { getStoredBuildings } from '../utils/assetDigitalSync';
...
const AssetDigitalization: React.FC<AssetDigitalizationProps> = ({ userRole, subView = 'land' }) => {
  ...
  const buildings: BuildingAsset[] = useMemo(() => {
    const stored = getStoredBuildings();
    const storedById = new globalThis.Map(stored.map(b => [b.id, b]));
    const merged: BuildingAsset[] = [...stored];

    // 合并 MOCK_BUILDINGS（作为默认数据），若本地已有同 id 则以本地为准
    MOCK_BUILDINGS.forEach(b => {
      if (storedById.has(b.id)) return;
      merged.push(b);
    });
  ...
  const rooms: RoomAsset[] = useMemo(() => {
    const stored = getStoredRooms();
    const storedById = new globalThis.Map(stored.map(r => [r.id, r]));
    const merged: RoomAsset[] = [...stored];

    // 合并 MOCK_ROOMS（作为默认数据），若本地已有同 id 则以本地为准
    MOCK_ROOMS.forEach(r => {
      if (storedById.has(r.id)) return;
      merged.push(r);
    });
  ...
  const displayedRooms = selectedBuildingId
    ? rooms.filter(r => r.buildingName === buildings.find(b => b.id === selectedBuildingId)?.name)
    : rooms;

  const getTitle = () => {
      switch(subView) {
          case 'land': return '土地资源管理';
          case 'building': return '房屋建筑管理';
          case 'room': return '房间原子单元管理';
          ...
          <div className="flex flex-col lg:flex-row gap-6 h-full">
             ...
             <div className="flex-[2] bg-[#1f2329] rounded-lg border border-[#1f2329] overflow-hidden flex flex-col relative min-h-[400px]">
                <div className="absolute top-4 left-4 z-10 bg-black/50 text-white px-3 py-1 rounded backdrop-blur-md text-sm">
                    {selectedBuildingId ? buildings.find(b => b.id === selectedBuildingId)?.name : '请选择楼宇'} - 图纸数字化视图
                </div>
